...
| File | URL |
|------|-----|
| **SKILL.md** (this file) | \`${baseUrl}/api/advisor/skill.md\` |
| **skill.json** (metadata) | \`${baseUrl}/api/advisor/skill.json\` |
...
\`\`\`bash
curl "${apiBase}/skills/search?q=QUERY&limit=5"
\`\`\`
...
**Example:**
\`\`\`bash
curl "${apiBase}/skills/search?q=web+scraping&limit=5&sort=relevance"
\`\`\`
  ...
  "results": [
    {
      ...
      "clawdtm_url": "${baseUrl}/skills/web-search"
    }
  ]
}
\`\`\`
...
\`\`\`bash
curl "${baseUrl}/api/v1/skills/install?slug=SKILL_SLUG"
\`\`\`
...
1. Search: \`GET ${apiBase}/skills/search?q={intent}&limit=5&sort=relevance\`
...
4. Fetch: \`GET ${baseUrl}/api/v1/skills/install?slug={chosen}\`
...
1. Search: \`GET ${apiBase}/skills/search?q={skill_name}&limit=1\`
...
1. Search: \`GET ${apiBase}/skills/search?q=&sort=downloads&limit=10\`
...
1. Search: \`GET ${apiBase}/skills/search?q={intent}&safe_only=true\`
2. This filters to only skills with \`low\` risk or better (score >= 70)

### "Show me all skills including risky ones for X"

1. Search: \`GET ${apiBase}/skills/search?q={intent}&include_risky=true\`
...
Fetch it at: \`${baseUrl}/api/review/skill.md\`
